...
- (void)testConnectInputStreamOutputStreamBufferSize2 {
    ...
    [self rcr_expectCondition:^BOOL {
        decryptedData = [self dataInOutputStream:outputStreamA];
        return decryptedData.length > 0;
    } beforeTimeout:5.0 interval:0.1 description:@"data was decrypted"];
    XCTAssertEqualObjects([self dataInOutputStream:outputStreamB], testData);
}

- (void)testConnectInputStreamOutputStreamBufferSize3 {
    RNEncryptor *cryptor = [[RNEncryptor alloc] initWithSettings:kRNCryptorAES256Settings password:@"password" handler:^(RNCryptor *cryptor, NSData *data) { }];
    NSInputStream *newInputStream = [[cryptor rcr_processInputStream:inputStream bufferSize:32*1024] first];
    [newInputStream scheduleInRunLoop:[NSRunLoop currentRunLoop] forMode:NSRunLoopCommonModes];
    [newInputStream open];
    NSOutputStream *newOutputStream = [[cryptor rcr_processOutputStream:outputStream bufferSize:32*1024] first];
    [newOutputStream scheduleInRunLoop:[NSRunLoop currentRunLoop] forMode:NSRunLoopCommonModes];
    [newOutputStream open];
    [self rcr_expectCondition:^BOOL {
        newInputStream;
        newOutputStream;
        return [[self dataInOutputStream:newOutputStream] length] > 0;
    } beforeTimeout:5.0 interval:0.1 description:@"data was encrypted"];
}
